...
class AutoHunting : public QThread {
  Q_OBJECT
public:
  AutoHunting(QObject *parent, QSharedPointer<VariablesClass> var,
              QSharedPointer<GameConnecter> gameConnector, Route route,
              Profile *profile);
  ~AutoHunting();
  void run();
...
private:
  ...
  QStringList alloweNamesOnBattleList = QStringList() << "Falcon Paladin"
                                                      << "PreceptorLazare"
                                                      << "Monk";
  ...
  bool playerFoundOnBattleList() {
    QStringList enemiesNamesOnBattleList =
        var->getBattleList().getUniqueMonstersNames();
  ...
  inline void playSound() {
    ...
    bool canAlreadyRangAlaram =
        currentTime >= lastTimeAlarmRang + breakBetweenAlarms;
    if (!canAlreadyRangAlaram)
      return;
