...
bool ImgEditor::isItPixelFromFrame(const uint& color, const int& minValueAcceptable, const int& maxValueAcceptable,
                                   bool requireSameValuesOfRGB) {
  RGBstruct rgb(color);

  int maxValueFound = rgb.getMaxColVal();
  if (maxValueFound > maxValueAcceptable) return false;

  int minValueFound = rgb.getMinColVal();
  if (minValueFound < minValueAcceptable) return false;

  if (requireSameValuesOfRGB) {
    if (maxValueFound - minValueFound > 1) return false;
  }
...
QList<QImage> ImgEditor::cutImgWithLettersToSingleLettersImgList(QImage& img) {
  ...
  for (size_t i = 0; i < indexesOfStartOfLetter.size(); i++) widths.push_back(indexesOfEndsOfLetters[i] - indexesOfStartOfLetter[i] + 1);
...
QImage ImgEditor::getColorImageFromCode(const QString& codeOfImg) {
  try {
    ...
    if (partsOfCode.size() != 3) throw std::exception(" Wrong format of color img code!");

    const int WIDTH  = partsOfCode[0].toInt();
    const int HEIGHT = partsOfCode[1].toInt();

    if (WIDTH <= 0 || HEIGHT <= 0) throw std::exception(" Wrong format of color img code!, sizesz should be positive!");

    const int HEX_LENGTH = 8;
    if (partsOfCode[2].size() % HEX_LENGTH != 0) throw std::exception(" Wrong format of color img code!, pixValues are not n*8 length!");

    QStringList hexValuesOfPixels = {};
    for (size_t i = 0; i <= partsOfCode[2].size(); i += HEX_LENGTH) hexValuesOfPixels.push_back(partsOfCode[3].mid(i, HEX_LENGTH));
      ...
      if (atLeastOnePixIsMatched) {
        ...
        for (int x_TMP = 0; x_TMP < WIDTH_OF_FIRST_IMG; x_TMP++) {
          for (int y_TMP = 0; y_TMP < HEIGHT_OF_FIRST_IMG; y_TMP++) {
            ...
            if (metReq) wrongPixels--;
          }
        }
        if (wrongPixels == 0) startPointsListToRet.push_back(QPoint(x, y));
      }
    }
  }
...
QPoint ImgEditor::findExactStartPositionInImg(const QImage& imgToFind, const QImage& imgToSearchWithin, QRect frameInBigWindow) {
  try {
    ...
    for (int x = minIndexToCheckX; x <= maxIndexToCheckX; x++) {
      for (int y = minIndexToCheckY; y <= maxIndexToCheckY; y++) {
        if (startPoints.size() > 1) throw std::exception("more than one point found in findExactStartPoint fun!");
    ...
    if (startPoints.size() != 1) throw std::exception("not exactly one position found in findExactStartPoint fun!");
