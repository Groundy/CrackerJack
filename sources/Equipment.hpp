...
public:
  ...
  QVector<STATES> getCurrentStates(bool clearImg = true) {
    ...
    QList<QImage> imgs =
        ImgEditor::cutImgWithLettersToSingleLettersImgList(stateBar);
  ...
  QString getEqRectBottomText(EqRect eqRect) {
    ...
    if (bottomStrImg.isNull())
      return QString();
    ImgEditor::imgToBlackAndWhiteExactColor(bottomStrImg,
                                            GREY_COL_OF_EQ_STRINGS);
    QString str = imgEditor->imgWithStrToStr(bottomStrImg);
    return str;
  }
  QRect getRect(EqRect eqRect) {
    const QPoint EQ_FRAME_TOPLEFT =
        storeRect.getRect().topLeft() - QPoint(74, 0);
    switch (eqRect) {
    case EqRect::Armor: {
    ...
    case EqRect::SoulPoints: {
      ...
      const QSize SOUL_AMOUNT_BAR_SIZE =
          QSize(32, 9); // not whole sqare but only amount!!!
      return QRect(start, SOUL_AMOUNT_BAR_SIZE);
    }
    case EqRect::Capacity: {
      QPoint start = EQ_FRAME_TOPLEFT + QPoint(75, 132);
      const QSize CAP_AMOUNT_BAR_SIZE =
          QSize(32, 9); // not whole sqare but only amount!!!
...
private:
  MutexImg statesBarImg, soulPtsImg, capImg, helmetCap, armorImg, legsImg,
      bootsImg, ringImg, amuletImg, weaponImg, shieldImg, backpackImg, torchImg;
  MutexRect storeRect;
  ImgEditor *imgEditor;
