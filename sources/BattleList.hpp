...
public:
  ...
  bool checkIfBattleListIsOk() {
    ...
    bool properImg =
        ImgEditor::findStartPositionInImg(battleMark, battleList).size() == 1;
    if (!properImg)
      setFrame(QRect());
    return properImg;
  }
  QStringList getUniqueMonstersNames() {
    ...
    for (int i = 0; i < nameRect.size(); i++) {
      ...
      QString monsterName =
          imgEditor->imgWithStrToStr(singleMonsterNameImg, 180);
...
private:
  ...
  QVector<QRect> getFramesOfMonstersNames() {
    ...
    int howManyNamesFitToListSize =
        ((innerRect.height() - NAME_HEIGHT) /
             (NAME_HEIGHT + HIGH_DIFF_BETWEEN_NAMES) +
         1); // one name + pairs of (name+space between names)
    for (int i = 0; i < howManyNamesFitToListSize; i++) {
      QPoint startPoint =
          INNER_FRAME_FIST_NAME_START +
          QPoint(0, i * (NAME_HEIGHT + HIGH_DIFF_BETWEEN_NAMES));
      toRet.append(QRect(startPoint.x(), startPoint.y(), innerRect.width(),
                         NAME_HEIGHT));
    }
    return toRet;
  };
