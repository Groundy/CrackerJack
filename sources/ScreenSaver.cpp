...
ScreenSaver::ScreenSaver(QObject* parent, QSharedPointer<VariablesClass> var,
    QSharedPointer<GameConnecter> gameConnecter, Profile* profile)
    : QThread(parent)
    , var(var)
...
ScreenSaver::~ScreenSaver() { this->terminate(); }
...
void ScreenSaver::sendScreenRequestToGame()
{
    try {
        ...
        if (pid == 0)
            throw std::exception(
                "Can't send screenshot key to game, there is no known pid!");
