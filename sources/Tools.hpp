 ...
 public:
  static void saveJsonFileWithCodesOfImgsInFolderColor(
    ...
    for each (QString fileName in litOfFIles) {
      ...
      QString strWithCode = QString("%1#%2#").arg(QString::number(WIDTH),
  ...
  static void saveJsonFileWithCodesOfImgsInFolderBinary(
      QString inputFolderPath,
      QString outputFolderPath) {
    // patern width_height_digits
    QDir directory(inputFolderPath);
  ...
  static void mergedMaps(QString inputPath, QString outputPath) {
    const int MIN_X_MAP_INDEX = 124, MAX_X_MAP_INDEX = 134,
              MIN_Y_MAP_INDEX = 121, MAX_Y_MAP_INDEX = 129;
    ...
    for each (QString type in types) {
      for (size_t floorImgName = 0; floorImgName <= 15; floorImgName++) {
        QImage* wholeImg = new QImage(WHOLE_IMG_WIDTH, WHOLE_IMG_HEIGHT,
                                      QImage::Format::Format_ARGB32);
        for (size_t xIndex = MIN_X_MAP_INDEX; xIndex <= MAX_X_MAP_INDEX;
             xIndex++) {
          for (size_t yIndex = MIN_Y_MAP_INDEX; yIndex <= MAX_Y_MAP_INDEX;
               yIndex++) {
            QString xValStr = QString::number(xIndex * 256);
            QString yValStr = QString::number(yIndex * 256);
            QString mapPieceFileName =
                QString("%1\\Minimap_%2_%3_%4_%5.png")
                    .arg(inputPath, type, xValStr, yValStr,
                         QString::number(floorImgName));
        ...
        QString outPutFileName =
            QString("%1\\%2_%3.png")
                .arg(outputPath, QString::number(realFloor), type);
        wholeImg->save(outPutFileName);
        delete wholeImg;
      }
    }
  }
  static void getAllColorsUsedInWalkableMaps(QString inputPath,
                                             QString outputPath) {
    QDir directory(inputPath);
    QStringList litOfFIles =
        directory.entryList(QStringList() << "*_Color*.png", QDir::Files);
