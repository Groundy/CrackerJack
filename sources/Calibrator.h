...
private:
  ...
  struct Indexes {
    ...
    bool isValid(int maxSize = 100) const {
      return healthFound(maxSize) &&
             (manaFound(maxSize) || combinedFound(maxSize));
    }
  };
  struct SlashesIndexes {
    ...
    bool isValid() {
      ...
      return ((xSize == 2 || xSize == 3) && ySize == 0) ||
  ...
  SlashesIndexes
  getIndexesOfImgsWithSlashes(const QImage &fullScreen,
                              const QList<QRect> &importantFrames);
  bool categorizeWindows(const QImage &fullscreen,
                         QList<QRect> &importantRectangles);
  bool findWindowsOnScreen(const QImage &fullScreen,
                           QList<QRect> &importantRectangles);
  void sortByXY(QList<QPoint> &points, QList<QPoint> &sortedByX,
                QList<QPoint> &sortedByY);
  void sortByXY(QList<QRect> &inputRects, QList<QRect> &sortedByX,
                QList<QRect> &sortedByY);
  Indexes
  getIndexesOfHealthManaBars(const QImage &fullscreen,
                             const QList<QRect> &listOfImportantRectangles);
  QList<QPoint> getStartOfPossibleFrames(const QImage &fullScreen, int minVal,
                                         int maxVal);
  QList<QRect> getAreasInsideFrames(const QImage &fullScreen,
                                    const QList<QPoint> &startOfFrames,
                                    const int MIN_DIM);
  QList<QRect>
  filterAreasCoveredByFrameFromBottomRight(const QImage &fullScreen,
                                           const QList<QRect> &areas);
  QList<QRect>
  getOutsideFramesOfOpenEntitiesOnSideBars(const QImage &wholeScreen);
};
