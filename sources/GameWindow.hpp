...
public:
  ...
  QRect getFrameMainGameWindow() { return gameWindowFrame.getRect(); }
  ...
  QVector<QPoint> getPointsOfFieldsAroundPlayer() {
    ...
    QPoint middleOfTitleZeroZero =
        frame.topLeft() + QPoint(TITLE_WIDTH / 2, TITLE_HEIGHT / 2);
    QPoint middleOfPlayerTitle =
        middleOfTitleZeroZero +
        QPoint(
            7 * TITLE_WIDTH,
            5 * TITLE_HEIGHT +
                12); // that way cursor is little too high, 12 to lower in down.
    QVector<QPoint> toRet = {};
    for (int x = -1; x <= 1; x++) {
      for (int y = -1; y <= 1; y++) {
        toRet.push_back(middleOfPlayerTitle +
                        QPoint(x * TITLE_WIDTH, y * TITLE_HEIGHT));
      }
    }
