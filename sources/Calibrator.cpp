...
bool Calibrator::calibrateBattleArea(const QImage &fullscreen) {
  ...
  for each (QRect rect in rectangles) {
    auto battleMarksPos =
        ImgEditor::findStartPositionInImg(battleListMark, fullscreen, rect);
...
bool Calibrator::calibrateStoreButton(const QImage &fullImage) {
  ...
  rect.setLeft(
      rect.width() *
      0.75); // shearch only witing last 25% of fulll screen, on right side
  QPoint storeButtonPosition =
      ImgEditor::findExactStartPositionInImg(img, fullImage, rect);
...
void Calibrator::test(QString pathToFilesWithScreens) {
  ...
  try {
    ...
    qDebug() << "avg time : " +
                    QString::number(totalTime / listOfFIlesNames.size());
...
// private
Calibrator::SlashesIndexes
Calibrator::getIndexesOfImgsWithSlashes(const QImage &fullScreen,
                                        const QList<QRect> &importantFrames) {
  ...
  for (size_t i = 0; i < importantFrames.size(); i++) {
    ...
    QList<QPoint> pointsX =
        ImgEditor::findStartPositionInImg(slashXImg, imgTmp);
    if (pointsX.size() == 1)
    ...
    QList<QPoint> pointsY =
        ImgEditor::findStartPositionInImg(slashYImg, imgTmp);
    if (pointsY.size() == 1)
...
Calibrator::Indexes Calibrator::getIndexesOfHealthManaBars(
    const QImage &fullscreen, const QList<QRect> &listOfImportantRectangles) {
  try {
    Indexes indexes;
    SlashesIndexes slashesIndexes =
        getIndexesOfImgsWithSlashes(fullscreen, listOfImportantRectangles);
    ...
    if (foundSlsahesY) {
      QRect firstRectangle =
          listOfImportantRectangles[slashesIndexes.slashesY[0]];
      position = firstRectangle.x() < midOfScreen.x() ? LEFT : RIGHT;
    } else {
      QRect firstRectangle =
          listOfImportantRectangles[slashesIndexes.slashesX[0]];
...
bool Calibrator::categorizeWindows(const QImage &fullscreen,
                                   QList<QRect> &importantRectangles) {
  ...
  // game Frame
  {
    ...
    for (size_t i = 0; i < importantRectangles.size(); i++) {
      int surf =
          importantRectangles[i].width() * importantRectangles[i].height();
  ...
  {
    Indexes indexes =
        getIndexesOfHealthManaBars(fullscreen, importantRectangles);
    ...
    if (indexes.combinedFound(size)) {
      var->getVitalitty().setCombinedArea(
          importantRectangles[indexes.combined]);
...
bool Calibrator::findWindowsOnScreen(const QImage &fullScreen,
                                     QList<QRect> &importantRectangles) {
  ...
  QList<QPoint> startOfFrames = getStartOfPossibleFrames(
      fullScreen, MIN_ACCEPTABLE_VAL, MAX_ACCEPTABLE_VAL);
  QList<QRect> areasWithoutFrames =
      getAreasInsideFrames(fullScreen, startOfFrames, 10);
  QList<QRect> areasWithinFullFrames =
      filterAreasCoveredByFrameFromBottomRight(fullScreen, areasWithoutFrames);
  importantRectangles = areasWithinFullFrames;
  return areasWithoutFrames.size() >= 4;
}
void Calibrator::sortByXY(QList<QPoint> &points, QList<QPoint> &sortedByX,
                          QList<QPoint> &sortedByY) {
...
void Calibrator::sortByXY(QList<QRect> &inputRects, QList<QRect> &sortedByX,
                          QList<QRect> &sortedByY) {
...
QList<QPoint> Calibrator::getStartOfPossibleFrames(const QImage &fullScreen,
  ...
  for (size_t x = 1; x < WIDTH - 2; x++) {
    for (size_t y = 1; y < HEIGHT - 2; y++) {
      bool isPixelOfFrame = ImgEditor::isItPixelFromFrame(
          fullScreen.pixel(x, y), MIN_ACCEPTABLE_VAL, MAX_ACCEPTABLE_VAL, true);
      if (!isPixelOfFrame)
        continue;

      isPixelOfFrame = ImgEditor::isItPixelFromFrame(
          fullScreen.pixel(x - 1, y - 1), MIN_ACCEPTABLE_VAL,
          MAX_ACCEPTABLE_VAL, false);
      if (isPixelOfFrame)
        continue;
      ...
      isPixelOfFrame = ImgEditor::isItPixelFromFrame(
          fullScreen.pixel(x + 1, y + 2), MIN_ACCEPTABLE_VAL,
          MAX_ACCEPTABLE_VAL, false);
      if (isPixelOfFrame)
        continue;

      isPixelOfFrame = ImgEditor::isItPixelFromFrame(
          fullScreen.pixel(x + 2, y + 1), MIN_ACCEPTABLE_VAL,
          MAX_ACCEPTABLE_VAL, false);
      if (isPixelOfFrame)
        continue;

      isPixelOfFrame = ImgEditor::isItPixelFromFrame(
          fullScreen.pixel(x + 2, y + 2), MIN_ACCEPTABLE_VAL,
          MAX_ACCEPTABLE_VAL, false);
      if (isPixelOfFrame)
        continue;

      isPixelOfFrame = ImgEditor::isItPixelFromFrame(fullScreen.pixel(x + 1, y),
                                                     MIN_ACCEPTABLE_VAL,
                                                     MAX_ACCEPTABLE_VAL, true);
      if (!isPixelOfFrame)
        continue;

      isPixelOfFrame = ImgEditor::isItPixelFromFrame(fullScreen.pixel(x + 2, y),
                                                     MIN_ACCEPTABLE_VAL,
                                                     MAX_ACCEPTABLE_VAL, true);
      if (!isPixelOfFrame)
        continue;

      isPixelOfFrame = ImgEditor::isItPixelFromFrame(fullScreen.pixel(x, y + 1),
                                                     MIN_ACCEPTABLE_VAL,
                                                     MAX_ACCEPTABLE_VAL, true);
      if (!isPixelOfFrame)
        continue;

      isPixelOfFrame = ImgEditor::isItPixelFromFrame(fullScreen.pixel(x, y + 2),
                                                     MIN_ACCEPTABLE_VAL,
                                                     MAX_ACCEPTABLE_VAL, true);
      if (!isPixelOfFrame)
        continue;
...
QList<QRect>
Calibrator::getAreasInsideFrames(const QImage &fullScreen,
                                 const QList<QPoint> &startOfFrames,
                                 const int MIN_DIM) {
  const int WIDTH = fullScreen.width();
  const int HEIGHT = fullScreen.height();
  const uint MIN_ACCEPTABLE_VAL = 17;
  ...
  for each (QPoint startPoint in startOfFrames) {
    ...
    for (size_t x = startPoint.x(); x < WIDTH; x++) {
      ...
      bool isPixOfFrame = ImgEditor::isItPixelFromFrame(
          color, MIN_ACCEPTABLE_VAL, MAX_ACCEPTABLE_VAL, true);
      if (isPixOfFrame)
        currentWidth++;
      else
        break;
...
QList<QRect> Calibrator::filterAreasCoveredByFrameFromBottomRight(
    const QImage &fullScreen, const QList<QRect> &areas) {
  const int WIDTH = fullScreen.width();
  const int HEIGHT = fullScreen.height();
  ...
  for each (QRect area in areas) {
    ...
    for (size_t x = 0; x < area.width(); x++) {
      ...
      bool framPix = ImgEditor::isItPixelFromFrame(color, MIN_ACCEPTABLE_VAL,
                                                   MAX_ACCEPTABLE_VAL, true);
      if (!framPix) {
        skip = true;
        break;
...
QList<QRect> Calibrator::getOutsideFramesOfOpenEntitiesOnSideBars(
    const QImage &wholeScreen) {
  const QImage startOfSideBarEntity(
      PathResource::getPathToSideBarEntityStart());
  if (startOfSideBarEntity.isNull())
    return {};

  QList<QRect> outerFramesOfSideBarsEntity = {};
  QList<QPoint> startPoints =
      ImgEditor::findStartPositionInImg(startOfSideBarEntity, wholeScreen);
  const QImage endOfSideBarEntity(PathResource::getPathToSideBarEntityEnd());
  for each (QPoint currPt in startPoints) {
    QRect rectToSheachWithIn(currPt, QSize(wholeScreen.width() - currPt.x(),
                                           wholeScreen.height() - currPt.y()));
    auto endsOfFrame = ImgEditor::findStartPositionInImg(
        endOfSideBarEntity, wholeScreen, rectToSheachWithIn);
    if (endsOfFrame.size() == 0)
      continue;
    QList<QPoint> sortedByX, sortedByY;
    sortByXY(endsOfFrame, sortedByX, sortedByY);
    QPoint pt = sortedByY[0];
    QSize size(pt.x() + endOfSideBarEntity.width() - currPt.x(),
               pt.y() + endOfSideBarEntity.height() - currPt.y());
