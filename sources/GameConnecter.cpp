...
GameConnecter::~GameConnecter() { releaseShift(); }
...
void GameConnecter::clickRightWithShift(QVector<QPoint> pts,
                                        int SLEEP_TIME_BETWEEN_LOOT_CLICK) {
  HWND gameThreadHandler = var->getGameProcess().getHandlerToGameThread();
...
void GameConnecter::sendCharToGame(const QChar charToSend,
                                   const HWND &gameThreadHandler) {
  senderMutex.lock();
